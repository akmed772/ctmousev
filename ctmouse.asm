...
DEBUGSCRWRITE	macro
	...
	mov	ax,ds
	mov	es,ax
	mov	ax,offset dbcsdebugtext
	mov	bp,ax
	mov dx,1600h
	mov	cx,80
	mov	bx,0007h
	mov	ax,1300h	; INT10h,AH=13h: read/write DBCS chars/attrs (DOS/V)
...
DEBUGPRINT4 macro val4,dcposx
	...
	pop	ax
endm
DEBUGWB macro val4,dcposx
	push si
	mov si,offset dbcsdebugtext
	add si,dcposx
	mov byte ptr [si],val4
	pop si
endm
bintohex proc
printhex:;al = 2-digit hexadecimal value, dh, dl = byte char
	push	ax
	shr	al, 1
	shr	al, 1
	shr	al, 1
...
; -X-		call	@disablePS2		; unhooked IRQ
		; to avoid corrupting order of buffer when mouse is moved during this sequence
		PS2serv	0C201h			; reset mouse and clear buffer
...
detisdosdbcs	proc
	...
	pop		ds
	or	ah,al
	jz	@@envnotdbcs	;ZF=1 if vector for DBCS lead-byte table is not set
@@envnotdbcs:
	pop	ax
	pop	si
	ret
